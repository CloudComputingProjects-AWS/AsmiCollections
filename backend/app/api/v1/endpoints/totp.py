...
SECURITY (Updated 01-Mar-2026 S16):
  - /2fa/validate now sets tokens as httpOnly cookies (not JSON body)
"""
...
async def validate_totp_login(
    ...
    SECURITY (S16): Tokens are set as httpOnly cookies, NOT returned in JSON body.
    """
    service = TwoFactorService(db)
    try:
...
def _generate_qr_base64(uri: str) -> str | None:
    ...
    try:
        ...
        # qrcode package not installed — return None, frontend can generate QR
        return None
