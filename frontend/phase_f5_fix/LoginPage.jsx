/**
 * LoginPage.jsx — Unified Login (Phase F5 Fix)
 *
 * Single login page for ALL roles (customer + all admin roles).
 ...
 *
 * Replaces both /login and /admin/login.
 */
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.password.trim()) return setLocalErr('Password is required');

    try {
      const profile = await login(form.email.trim(), form.password);

      // Honor ?next= param (relative paths only, security check)
      ...
      // error set in store — displayed via `error` binding above
    }
  };
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
          ...
          {/* Error */}
          {displayError && (
            <div className="mb-5 px-4 py-3 bg-red-50 border border-red-200 rounded-lg text-red-700 text-sm flex gap-2 items-start">
          ...
          {/* Dev hint */}
          {import.meta.env.DEV && (
            <div className="mt-6 p-3 bg-gray-50 rounded-lg border border-dashed border-gray-200 text-xs text-gray-400 space-y-0.5">
              <p className="font-medium text-gray-500 mb-1">Dev — role routing:</p>
              {Object.entries(ROLE_DEFAULT_ROUTE).map(([role, route]) => (
                <p key={role}><span className="text-blue-500">{role}</span> → {route}</p>
              ))}
            </div>
          )}
        </div>
