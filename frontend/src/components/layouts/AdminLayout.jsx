/**
 * Admin Layout ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Production (components/layouts/)
 *
...
import {
  ...
  LogOut, Menu, X, Receipt, BarChart3, Settings, ChevronDown, User,
} from 'lucide-react';
...
/**
 * NAV_ITEMS ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â must match App.jsx admin routes exactly.
 ...
 */
const NAV_ITEMS = [
  { label: 'Dashboard', icon: LayoutDashboard, path: '/admin', roles: ['admin', 'product_manager', 'order_manager', 'finance_manager'] },
  { label: 'Products', icon: Package, path: '/admin/products', roles: ['admin', 'product_manager'] },
  { label: 'Categories', icon: FolderTree, path: '/admin/categories', roles: ['admin', 'product_manager'] },
  { label: 'Attributes', icon: Tags, path: '/admin/attributes', roles: ['admin', 'product_manager'] },
  { label: 'Inventory', icon: Layers, path: '/admin/inventory', roles: ['admin', 'product_manager'] },
  { label: 'Orders', icon: ShoppingCart, path: '/admin/orders', roles: ['admin', 'order_manager'] },
  { label: 'Returns', icon: RefreshCcw, path: '/admin/returns', roles: ['admin', 'order_manager'] },
  { label: 'Invoices', icon: FileText, path: '/admin/invoices', roles: ['admin', 'finance_manager'] },
  { label: 'Coupons', icon: Receipt, path: '/admin/coupons', roles: ['admin', 'product_manager'] },
  { label: 'Reports', icon: BarChart3, path: '/admin/reports', roles: ['admin', 'finance_manager'] },
  ...
  return (
    <>
      ...
      <div className="px-4 py-3 border-t border-ink-inverse/10">
        ...
        <button
          onClick={async () => { await logout(); navigate('/login', { state: { logoutSuccess: true } }); }}
    ...
    <div className="flex items-center justify-between px-4 py-3 bg-surface-raised border-b border-ink-faint/10 lg:px-6">
      ...
      <div className="relative" ref={dropdownRef}>
        <button
          ...
          className="flex items-center gap-2 text-sm font-medium text-ink-muted hover:text-ink transition-colors px-3 py-1.5 rounded-md hover:bg-ink-faint/5"
